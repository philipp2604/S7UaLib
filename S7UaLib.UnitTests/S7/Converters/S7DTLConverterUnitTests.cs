...
public class S7DTLConverterUnitTests
{
    ...
    public S7DTLConverterUnitTests()
    {
        _mockLogger = new Mock<ILogger>();
    }

    private S7DTLConverter CreateSut()
    {
        return new S7DTLConverter(_mockLogger.Object);
    }
    ...
    public void ConvertFromOpc_WhenValueIsNotExtensionObject_ReturnsNullAndLogsError()
    {
        ...
        _mockLogger.Verify(
            log => log.Log(
                LogLevel.Error,
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, _) => v.ToString()!.Contains("but expected 'Opc.Ua.ExtensionObject'")),
                null,
    ...
    public void ConvertFromOpc_WhenTypeIdIsIncorrect_ReturnsNullAndLogsError()
    {
        ...
        _mockLogger.Verify(
            log => log.Log(
                LogLevel.Error,
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, _) => v.ToString()!.Contains($"but expected '{_s7DtlTypeId}'")),
                null,
                It.IsAny<Func<It.IsAnyType, Exception?, string>>()),
            Times.Once);
    }
    ...
    public void ConvertFromOpc_WhenByteArrayIsWrongLength_ReturnsNullAndLogsError()
    {
        ...
        _mockLogger.Verify(
            log => log.Log(
                LogLevel.Error,
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, _) => v.ToString()!.Contains("must have a length of 12")),
                null,
                It.IsAny<Func<It.IsAnyType, Exception?, string>>()),
            Times.Once);
    }
    ...
    public void ConvertFromOpc_WhenDateComponentIsInvalid_ReturnsNullAndLogsError()
    {
        ...
        _mockLogger.Verify(
            log => log.Log(
                LogLevel.Error,
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, _) => v.ToString()!.Contains("Invalid date or time component")),
                null,
                It.IsAny<Func<It.IsAnyType, Exception?, string>>()),
            Times.Once);
    }
    ...
    public void ConvertToOpc_WithYearOutsideRange_ReturnsValueAndLogsWarning()
    {
        ...
        _mockLogger.Verify(
            log => log.Log(
                LogLevel.Warning,
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, _) => v.ToString()!.Contains("is outside the typical S7 DTL range")),
                null,
                It.IsAny<Func<It.IsAnyType, Exception?, string>>()),
            Times.Once);
    }
