...
public class S7ArrayConverterUnitTests
{
    ...
    public S7ArrayConverterUnitTests()
    {
        _mockLogger = new Mock<ILogger>();
    }

    private S7ArrayConverter<T> CreateSut<T>()
    {
        return new S7ArrayConverter<T>(_mockLogger.Object);
    }
    ...
    public void ConvertFromOpc_WhenValueIsIncompatibleType_ReturnsNullAndLogsError()
    {
        ...
        _mockLogger.Verify(
            log => log.Log(
                LogLevel.Error,
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, _) => v.ToString()!.Contains($"but expected '{typeof(int).FullName}[]'")),
                null,
    ...
    public void ConvertToOpc_WhenValueIsIncompatibleType_ReturnsNullAndLogsError()
    {
        ...
        _mockLogger.Verify(
            log => log.Log(
                LogLevel.Error,
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, _) => v.ToString()!.Contains($"but expected a compatible collection of '{typeof(int).FullName}'")),
                null,
                It.IsAny<Func<It.IsAnyType, Exception?, string>>()),
            Times.Once);
    }
