...
public class S7DateConverterUnitTests
{
    ...
    public S7DateConverterUnitTests()
    {
        _mockLogger = new Mock<ILogger>();
    }

    private S7DateConverter CreateSut()
    {
        return new S7DateConverter(_mockLogger.Object);
    }
    ...
    public void ConvertToOpc_WithDateBeforeEpoch_ReturnsNullAndLogsError()
    {
        ...
        _mockLogger.Verify(
            log => log.Log(
                LogLevel.Error,
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, _) => v.ToString()!.Contains("Date is outside the valid S7 range")),
                null,
                It.IsAny<Func<It.IsAnyType, Exception?, string>>()),
            Times.Once);
    }
